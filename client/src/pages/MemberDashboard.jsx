                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  ...
                  >
                    {(selectedMemberInfo.user_profiles?.full_name?.charAt(0) ||
                      selectedMemberInfo.user_profiles?.email?.charAt(0) ||
                      "?"
                    ).toUpperCase()}
                  </div>
                  <div>
    ...
    try {
      ...
      try {
        ...
        if (mountedRef.current) {
          setMyFeedback([]);
          setMyStats({ totalFeedback: 0, avgRating: 0, recentActivity: 0, acceptanceRate: 0 });
        }
      }
  ...
  const renderFeedbackRow = (feedback, index) => {
    ...
    return (
      <tr key={feedback.id || index} style={{ ...styles.tableRow }}>
        <td style={{ padding: "18px", verticalAlign: "top", width: 180 }}>
          <div style={{ display: "inline-block", padding: "8px 12px", borderRadius: 999, background: statusColor, color: "#fff", fontWeight: 700 }}>
            {feedback.decision === "accepted" ? "✅ Accepted" : feedback.decision === "rejected" ? "❌ Rejected" : "⏳ Pending"}
          </div>
        </td>
        ...
        <td style={{ padding: "18px", verticalAlign: "top", maxWidth: 320 }}>
          {feedback.suggestion ? (
            <div style={{ borderRadius: 12, padding: 12, background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.06)" }}>
              <div style={{ color: "#e6eef8", fontSize: 14 }}>{feedback.suggestion.length > 120 ? feedback.suggestion.substring(0, 120) + "…" : feedback.suggestion}</div>
            </div>
          ) : (
            <span style={{ color: "#94a3b8", fontStyle: "italic" }}>No suggestion text</span>
          )}
        </td>

        <td style={{ padding: "18px", verticalAlign: "top", maxWidth: 300 }}>
          {feedback.comment ? (
            <div style={{ borderRadius: 12, padding: 12, background: "rgba(236,72,153,0.04)", border: "1px solid rgba(236,72,153,0.04)" }}>
              <div style={{ color: "#e6eef8", fontSize: 14 }}>{feedback.comment.length > 100 ? feedback.comment.substring(0, 100) + "…" : feedback.comment}</div>
            </div>
          ) : (
        ...
        <td style={{ padding: "18px", verticalAlign: "top", textAlign: "center", width: 180 }}>
          <div style={{ borderRadius: 12, padding: 10, background: "rgba(255,255,255,0.02)" }}>
            <div style={{ fontSize: 12, color: "#94a3b8" }}>Lang</div>
            <div style={{ fontWeight: 700, color: "#ffd8a8" }}>{(feedback.language || "Unknown").toUpperCase()}</div>
            <div style={{ height: 8 }} />
            <div style={{ fontSize: 12, color: "#94a3b8" }}>Source</div>
            <div style={{ fontWeight: 700, color: "#c7f9cc" }}>{(feedback.source || "Static").charAt(0).toUpperCase() + (feedback.source || "Static").slice(1)}</div>
          </div>
        </td>
      </tr>
    );
        ...
        <div style={{ marginBottom: 28 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 18 }}>
            <div style={{ ...styles.statCard }} {...attachHoverTransformHandlers()}>
              ...
              <div style={{ color: "#94a3b8", marginTop: 6 }}>Total Feedback Submitted</div>
            ...
            <div style={{ ...styles.statCard }} {...attachHoverTransformHandlers()}>
              ...
              <div style={{ color: "#94a3b8", marginTop: 6 }}>Acceptance Rate</div>
              <div style={{ ...styles.progressBarOuter }}>
            ...
            <div style={{ ...styles.statCard }} {...attachHoverTransformHandlers()}>
              ...
              <div style={{ color: "#94a3b8", marginTop: 6 }}>Recent Activity (7 days)</div>
              <div style={{ ...styles.progressBarOuter }}>
        ...
        <div style={{ marginBottom: 30, ...styles.glassCard }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <div>
                <div style={{ fontSize: 20, fontWeight: 800, color: "#e6eef8" }}>Recent Feedback</div>
                <div style={{ color: "#94a3b8", fontSize: 13 }}>Latest entries you submitted</div>
              </div>
            </div>
          ...
          {myFeedback.length > 0 ? (
              ...
              <table style={{ ...styles.table }}>
                <thead>
                  <tr>
                    ...
                    <th style={{ ...styles.tableHeaderCell, textAlign: "center" }}>⚙️ Technical</th>
                  </tr>
                </thead>
                <tbody>
                  {myFeedback.slice(0, 5).map((f, idx) => renderFeedbackRow(f, idx))}
                </tbody>
              </table>
            ...
            <div style={{ padding: 28, textAlign: "center" }}>
              ...
              <div style={{ fontSize: 22, fontWeight: 800, marginBottom: 8 }}>No Feedback History</div>
              <div style={{ color: "#94a3b8", marginBottom: 14 }}>You haven't submitted any feedback yet</div>
              <div style={{ background: "rgba(99,102,241,0.04)", padding: 14, borderRadius: 12 }}>
                <div style={{ fontWeight: 700, color: "#c7d2fe", marginBottom: 6 }}>Pro Tip</div>
                <div style={{ color: "#c7d2fe" }}>Use the code analysis feature to provide feedback and suggestions to your team members. Your insights help improve code quality and team collaboration.</div>
              </div>
            </div>
          )}
        </div>
              ...
              <div style={{ marginBottom: 12, padding: 12, borderRadius: 12, background: "linear-gradient(90deg,#fbbf24,#fb923c)", backgroundBlendMode: "multiply", color: "#fff" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 64, height: 64, borderRadius: 999, background: "linear-gradient(135deg,#f59e0b,#fb923c)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color: "#fff", fontSize: 22 }}>
                    {(owner.user_profiles?.full_name?.charAt(0) || owner.user_profiles?.username?.charAt(0) || "👑").toUpperCase()}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 800, fontSize: 18 }}>{owner.user_profiles?.full_name || owner.user_profiles?.username || `Owner ${owner.user_id?.substring(0, 8)}`}</div>
                    <div style={{ color: "#fff", opacity: 0.9 }}>📧 {owner.user_profiles?.email || `ID: ${owner.user_id?.substring(0, 12)}`}</div>
                  </div>
                </div>
              </div>
            );
                      ...
                      <tr key={member.user_id || idx} style={{ ...styles.tableRow }}>
                        <td style={{ padding: 12 }}>
                          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                            <div style={{ width: 48, height: 48, borderRadius: 999, background: "linear-gradient(135deg,#6366f1,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontWeight: 800 }}>
                              {(member.user_profiles?.full_name?.charAt(0) || member.user_profiles?.username?.charAt(0) || member.user_profiles?.email?.charAt(0) || "?").toUpperCase()}
                            </div>
                            <div>
                              <div style={{ fontWeight: 700 }}>{member.user_profiles?.full_name || member.user_profiles?.username || member.user_profiles?.email?.split?.("@")?.[0] || `Member ${member.user_id?.substring(0, 8)}`}</div>
                            </div>
                          </div>
                        </td>
                        ...
                        <td style={{ padding: 12 }}>
                          <div style={{ background: "rgba(255,255,255,0.02)", padding: "6px 8px", borderRadius: 8, display: "inline-block", color: "#fff", fontSize: 13 }}>
                            📧 {member.user_profiles?.email || `ID: ${member.user_id?.substring(0, 12)}`}
                          </div>
                        </td>
