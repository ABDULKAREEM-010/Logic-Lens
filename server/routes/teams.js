...
router.post('/', verifyUserToken, async (req, res) => {
  ...
  try {
    ...
    const join_link = `${process.env.CLIENT_URL || 'http://localhost:5173'}/join/${team.team_id}`;
...
router.get('/:teamId/members', verifyUserToken, async (req, res) => {
  ...
  console.log('=== GET /teams/:teamId/members ===');
  ...
  try {
    ...
    // Get user profiles from Supabase auth for all members
      ...
      members.map(async (member) => {
        try {
          ...
          if (userData?.user && !userError) {
            ...
            console.log(`✅ Got real user data for ${member.user_id}:`, userProfile);
          } else {
            ...
            console.log(`⚠️  Using fallback data for ${member.user_id}:`, userError?.message);
          }
          ...
          console.error(`Error fetching user profile for ${member.user_id}:`, error);
